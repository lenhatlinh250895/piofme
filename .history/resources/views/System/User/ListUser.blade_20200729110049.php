...
<script>
    ...
    $('.btn-submit').on('click', function(){
        ...
        var password_confirm = $('.inp-password-confirm').val();
        var level = $('.inp-level').val();
        $.ajax({
            type: 'POST',
            url: "{{ route('system.user.postAjaxEdit')}}",
            data: {_token: token, id: id, name: name, email: email, password: password, password_confirm: password_confirm, level: level},
            dataType: 'JSON',
            success: function($result){
                console.log($result);
                console.log($result['id']);
