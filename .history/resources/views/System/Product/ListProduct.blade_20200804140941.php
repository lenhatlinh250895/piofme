...
<div class="container-fluid">
    ...
    <ul class="nav nav-tabs" id="myTab" role="tablist">
        ...
        <li class="nav-item">
            <a class="nav-link" id="la->id" data-toggle="tab" href="#add-product" role="tab" aria-controls="home" aria-selected="true">ADD PRODUCT</a>
        </li>
    </ul>
    <div class="tab-content" id="myTabContent">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <select class="form-control" name="service_search">
                                            ...
                                            <option value="{{$ser->id}}" @if(request()->input('service_search') == $ser->id) selected @endif>{{$ser->title}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
        ...
        <div class="tab-pane fade in" id="add-product" role="tabpanel" aria-labelledby="1-tab">
            <div class="row">
                <div class="col-12">
                    <div class="card m-b-30">
                        <div class="card-body">
                            <form action="{{ route('system.product.postAddProduct') }}" method="post"  enctype="multipart/form-data">
                                @csrf
                                <div class="form-group row">
                                    <label for="example-text-input" class="col-sm-2 col-form-label">Name</label>
                                    <div class="col-sm-10">
                        ...
                        <table class="table" id="my-table">
                            ...
                            <tbody>
                                ...
                                <tr id="id_{{$item->id}}">
                                    ...
                                    <td class="tabledit-view-mode"><span class="tabledit-span">{{ $item->service->title }}</span></td>
                                    <td style="white-space: nowrap; width: 15%;"><div class="tabledit-toolbar btn-toolbar" style="text-align: left;">
                                    @if($item->level != 1)
                                    <div class="btn-group btn-group-sm" style="float: none;">
                                        <a href="{{ Route('system.product.getEditProduct',$item->id) }}"><button type="button" data-toggle="modal" data-target="#modalEditUser" data-id="{{$item->id}}" class="btn-edit tabledit-edit-button btn btn-sm btn-light" style="float: none; margin: 5px;"><span class="ti-pencil"></span></button></a>
    ...
    $('.btn-delete').on('click', function(){
        ...
        confirmButtonText: 'Yes, deleit!'
            ...
            if (result.value) {
                $.ajax({
                    type: 'GET',
                    url: "{{ route('system.product.getDeleteProduct')}}",
